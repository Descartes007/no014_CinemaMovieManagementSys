      ...
      <el-main>
        ...
        <div class="item2" v-for="(item, index) in leavingMessageList" :key="index">
          <div style="display: flex">
            <img v-if="item.user && item.user.nickname" class="av2" :src="item.user.avatar" alt="">
            <div>
              <div v-if="!item.user || !item.user.nickname" class="d2">匿名用户</div>
...
export default {
  ...
  mounted() {
    ...
    ListAllLeavingMessage().then(res => {
      ...
      console.log(this.leavingMessageList, 444);
    })
    ListAllWorker().then(res => {
      this.workerList = res.data
      console.log(this.workerList, 333);
    })
  },
